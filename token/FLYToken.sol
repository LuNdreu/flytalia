...
contract FLYToken is ERC20, Ownable {
        ...
        ERC20(name, symbol)
    {
        ...
        if (initialSupply <= cap) {
            ...
            new_cap=cap-initialSupply;
        } else {_mint(owner(), cap);}
    }

    /**
     * @dev Tokens can be minted only if less than what is left to reach the cap.
     */
    modifier canMint(uint256 value) {
        require(value<=new_cap, "You are trying to mint more than what is left!");
        _;
    }
     ...
     */
    function mint(address to, uint256 value) public onlyOwner canMint(value) {
        _mint(to, value);
        new_cap=new_cap - value;
    } 
